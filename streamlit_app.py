...
def login():
    if st.session_state.get("pwd") == PASSWORD:
        st.session_state.authenticated = True

if not st.session_state.authenticated:
    st.title("🔒 비밀번호 입력")
    st.text_input("비밀번호를 입력하세요", type="password", key="pwd", on_change=login)
    if not st.session_state.authenticated:
        st.stop()

st.title("🛠️ STL → G‑code 컨버터")
st.markdown("STL 파일 업로드 후 파라미터를 조정하세요. 문의: 주창우 부장 (010-6754-2575)")
...
def generate_gcode(mesh,
                   ...
                   m30_on=False):
    extrusion_k = 0.05
    g = ["; *** Generated by STL→G‑code Exporter ***", "G21", "G90"]
    if e_on:
        g.append("M83")
...
if uploaded is not None:
    ...
    try:
        ...
        # 유효 face 필터링
        valid_faces = [face for face in mesh.faces if all(0 <= i < len(mesh.vertices) for i in face)]
        mesh.faces = np.array(valid_faces)
        if len(mesh.faces) == 0:
            st.error("❌ 유효한 삼각형 face가 없습니다.")
            st.stop()
