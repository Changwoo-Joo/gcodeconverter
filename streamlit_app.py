...
if not st.session_state.authenticated:
    st.title("🔒 비밀번호 입력")
    st.text_input("비밀번호를 입력하세요", type="password", key="pwd", on_change=authenticate)
    if not st.session_state.authenticated:
        st.stop()
...
def generate_gcode(mesh,
    ...
    z_values = list(np.arange(z_int, z_max + 0.0001, z_int))
...
if uploaded is not None:
    ...
    try:
        ...
        valid_faces = [face for face in mesh.faces if len(face) == 3 and all(0 <= i < num_vertices for i in face)]
        mesh.faces = np.array(valid_faces)
        if len(mesh.faces) == 0:
            raise ValueError("유효한 face가 없음")
