...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
                   ...
                   e0_on=False, trim_dist=30.0, min_spacing=3.0,
    ...
    for z in np.arange(z_int, z_max + z_int / 2, z_int):
...
if uploaded is not None:
    ...
    try:
        ...
        # 유효하지 않은 face 제거
        mesh.faces = np.array([f for f in mesh.faces if all(i < len(mesh.vertices) for i in f) and len(f) == 3])
        if len(mesh.faces) == 0:
            raise ValueError("유효한 face가 없습니다.")

        # Z 최대면에서 0.01mm 돌출된 얇은 판 자동 추가
        z_max = mesh.bounds[1][2]
        tol = 1e-5
        top_faces = []
        for f in mesh.faces:
