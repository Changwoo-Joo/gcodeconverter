...
def login():
    st.title("🔒 비밀번호 입력")
    pwd = st.text_input("비밀번호를 입력하세요", type="password", on_change=st.rerun, key="pwd_input")
    if st.session_state.get("pwd_input") == PASSWORD:
        st.session_state.authenticated = True
        st.rerun()
    elif st.session_state.get("pwd_input") and st.session_state.get("pwd_input") != PASSWORD:
...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
    ...
    z_range = np.arange(z_int, z_max + 0.001, z_int)
    prev_start_xy = None
...
if uploaded is not None:
    ...
    try:
        ...
        for f in mesh.faces:
            if len(f) != 3 or any(i >= num_vertices for i in f):
                continue
