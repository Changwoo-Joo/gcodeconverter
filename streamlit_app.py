...
if not st.session_state.authenticated:
    st.title("🔒 비밀번호 입력")
    st.text_input("비밀번호를 입력하세요", type="password", key="password_input", on_change=check_password_entered)
    if st.button("로그인"):
...
def generate_gcode(mesh,
    ...
    for z in np.arange(z_int, z_max + 0.001, z_int):
...
if uploaded is not None:
    ...
    try:
        ...
        for f in mesh.faces:
            ...
            if len(f) != 3 or np.any(f >= num_vertices):
                continue
