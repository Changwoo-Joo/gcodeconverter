...
st.markdown("""
STL파일을 업로드해주세요. 좌측 파라미터에서 Z값과 속도, 시작점을 지정해주세요.
옵션을 지정하면 지정된 옵션에 따라 경로가 생성됩니다.  
궁금하신 사항은 동아로보틱스 기술연구소 주창우부장(010-6754-2575)로 연락해주세요.
""")
...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0), e_on=False,
                   ...
                   min_spacing=3.0, auto_start=False, m30_on=False):
    extrusion_k = 0.05
    ...
    for z in np.arange(z_int, z_max + 0.1, z_int):  # +0.1 로 정확히 max 포함
...
if uploaded is not None:
    ...
    try:
        ...
        if mesh.faces is not None:
            valid_faces = [f for f in mesh.faces if max(f) < num_vertices and len(f) == 3]
            mesh.faces = np.array(valid_faces)

        if mesh.faces is None or len(mesh.faces) == 0:
            raise ValueError("STL에 유효한 face 정보가 없습니다.")
